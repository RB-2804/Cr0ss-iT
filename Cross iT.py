...
def remplissage_table_eleves():
    ...
    for i in range(len(liste)):
        cur = conn.cursor()
        cur.execute("INSERT INTO eleves (nom, classe, sexe) VALUES (?, ?, ?) ;", (liste[i][0], liste[i][1],'Masculin'))
...
def affiche_classement(lst_participants, surface, x, y, hauteur, couleur_bord, couleur_texte, taille_text) :
    long = 0
    for el in lst_participants :
        if el > long :
            long = el
        pygame.draw.rect(surface, couleur_bord, (x, y, LARGEUR, hauteur))
        font = pygame.font.Font(None, taille_text)
        text = font.render(el, True, couleur_texte)
        surface.blit(text, (x + long // 2 - text.get_width() // 2, y + hauteur // 2 - text.get_height() // 2))
...
while True:
    for event in pygame.event.get():
        ...
        if etat == ACCUEIL_SC:
            affichage(BCKGR_COLOR, GRIS, GRIS, GRIS)
        ...
        if etat == INSCRIPTIONS_SC:
            ...
            affichage(GRIS, BCKGR_COLOR, GRIS, GRIS)
        ...
        if etat == DEPART_SC:
            ...
            affichage(GRIS, GRIS, BCKGR_COLOR, GRIS)
            ...
            if 900 < mouse_x < 1125 and 100 < mouse_y < 125 and course_id_tableaux != None and nom_course != None:
                ...
                for resul in liste_resultat:
                    if premier:
                        ...
                        requete_insert("INSERT INTO participe VALUES (?, ?, ?, ?) ",
                                       (course_id_tableaux, resul[0], resul[1], \
                                        requete(f"SELECT MAX(classement)+1 FROM participe WHERE id_course = \
                                                                                                          (SELECT id_course FROM course WHERE nom = {nom_course}")))
            ...
            if choisir_course:
                ...
                for i in range(len(eleves)):
                    ...
                    #create_classement(surface, x, y, lst_participants, couleur_bord, couleur_texte, taille_text)

        if etat == COURSE_SC:

            affichage(GRIS, GRIS, GRIS, BCKGR_COLOR)
